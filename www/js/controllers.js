...
var configreload = {};
angular.module('jalsa.controllers', ['jalsa.services', 'ion-gallery', 'ngCordova', 'ngSanitize'])
	...
	configreload.func = function () {
		...
		_.each(data.menu, function (n) {
			if (loginstatus == false) {
				if (n.linktypelink != "setting" && n.linktypelink != "contact" ) {
					var newmenu = {};
					newmenu.id = n.id;
					newmenu.name = n.name;
...
.controller('LoginCtrl', function ($scope, MyServices, $ionicPopup, $interval, $location, $window, $ionicLoading, $timeout) {
	...
	$scope.setup = function () {$scope.logindata.email = true;
		$scope.config = MyServices.getconfigdata();
...
.controller('HomeCtrl', function ($scope, $location, $window, MyServices, $ionicLoading, $timeout, $sce, $ionicSlideBoxDelegate) {
	...
	MyServices.getevents(function (data) {
	    ...
	    $.jStorage.set("event_active",data.data.event); 
	}, function (err) {
		//$location.url("/access/offline");
	});
	$scope.library = []; $scope.lib_files = []; 
	$scope.totalCount = 0;
	$scope.countInit = function() {
	   return $scope.totalCount++;
	}
	MyServices.getlibrary($.jStorage.get("event_active"), function (data, status) {
			if(data.response == "success"){
				_.forEach(data.data, function (value, key) { 
				$scope.library.push(key);
				$scope.lib_files.push(value);
			});
			
			/*_.each(data.data.my_tasks, function (n) { 
				$scope.mytask.push(n);
			});*/
			}
		}, function (err) {
	...
	$scope.eventview = function (id) {
		if(id == "chart"){
			$location.url("app/org-chart");
		}
		if(id == "meetings"){
			$location.url("app/meetings");
		}
		if(id == "redbook"){
			$location.url("app/redbook");
		}
		if(id == "tasks"){
			$location.url("app/tasks");
		}
		if(id == "action-points"){
			$location.url("app/action-points");
		}
		if(id == "communications"){
			$location.url("app/communications");
		}
	}
	
	$scope.viewlibrary = function (id) {
		$(".librarylist, .home-tab").hide();
		$(".libview").find('[data-id="'+id+'"]').show();
		$(".libview").find('[data-id="'+id+'"]').parent().show();
		$(".libname").show().find('a span').empty().append(id);
	}
	
	$scope.showhome = function(){
		$(".librarylist, .home-tab").show();
		$(".libname").hide();
		$(".libview .item-text-wrap").hide();
	};
	
})

.controller('OrgCtrl', function ($scope, MyServices, $ionicModal) {
	var org = $.jStorage.get("event_active");
	MyServices.getchart(org, function (data, status) {
			if(data.response == "success"){
...
.controller('MeetingCtrl', function ($scope,$location, MyServices, $ionicModal, $cordovaFileTransfer) { 
	$scope.events = []; 
	MyServices.getmeeting($.jStorage.get("event_active"), function (data, status) {
			if(data.response == "success"){
  ...
  $scope.getinvitee = function(id){
	  $location.url("/app/invitee/" + id);
  }
	
})


.controller('InviteeCtrl', function ($scope,$location, $stateParams, MyServices, $ionicLoading) {
	configreload.onallpage();
	$scope.showloading = function () {
		$ionicLoading.show({
			template: '<ion-spinner class="spinner-positive"></ion-spinner>'
	...
	$scope.msg = "Loading...";
	$scope.invite = []; 
	MyServices.getinvitees($stateParams.id, function (data, status) {
			if(data.response == "success"){
				_.each(data.data.emem, function (n) { 
				$scope.invite.push(n);
			});
				_.each(data.data.allmem, function (n) { 
				$scope.invite.push(n);
			});
			
			_.each(data.data.c_mem, function (n) { 
				$scope.invite.push(n);
			});
...
.controller('RedbookCtrl', function ($scope,$location, MyServices, $ionicModal, $cordovaFileTransfer) { 
	$scope.redbooks = []; 
	MyServices.getredbook($.jStorage.get("event_active"), function (data, status) {
			if(data.response == "success"){
				_.each(data.data, function (n) { 
				$scope.redbooks.push(n);
			});
			}
		}, function (err) {
			$location.url("/access/offline");
...
.controller('TasksCtrl', function ($scope,$location, MyServices, $ionicModal, $cordovaFileTransfer) { 
	$scope.mytask =[]; $scope.assigntask = []; 
	MyServices.gettasks($.jStorage.get("event_active"), function (data, status) {
			if(data.response == "success"){
				_.each(data.data.ass_task, function (n) { 
				$scope.assigntask.push(n);
			});
			_.each(data.data.my_tasks, function (n) { 
				$scope.mytask.push(n);
			});
...
.controller('ActionspointsCtrl', function ($scope,$location, MyServices, $ionicModal, $cordovaFileTransfer) { 
	$scope.myaction =[]; $scope.assignaction = []; 
	MyServices.getactions($.jStorage.get("event_active"), function (data, status) {
			if(data.response == "success"){
				_.each(data.data.ass_action, function (n) { 
				$scope.assignaction.push(n);
			});
			_.each(data.data.my_actions, function (n) { 
				$scope.myaction.push(n);
			});
			}
		}, function (err) {
			$location.url("/access/offline");
...
.controller('CommunicationsCtrl', function ($scope, MyServices, $ionicModal, $ionicLoading) {
	var org = $.jStorage.get("event_active");
	$scope.mail =[]; $scope.csms = []; 
	MyServices.getcommunications(org, function (data, status) {
			if(data.response == "success"){
				$scope.mail = data.data.cmail;
				$scope.csms = data.data.csms;
			}
		}, function (err) {
			$location.url("/access/offline");
		})
